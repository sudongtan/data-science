...
def update_phone_number(phone_number):
    """Upadte the phone numbers to standard format"""
...
def shape_element(element, node_attr_fields=NODE_FIELDS, way_attr_fields=WAY_FIELDS,
    ...
    if element.tag == 'node':
        ...
        for child in element:
            ...
            if PROBLEMCHARS.match(ktag):
                continue
            
            node_tag['id'] = element.attrib['id']
            node_tag['value'] = child.attrib['v']

            if LOWER_COLON.match(ktag):
                x = ktag.find(":")
                node_tag['key'] = ktag[x+1:] 
                node_tag['type'] = ktag[: x+1]
            ...
            if is_street_name(child):
                node_tag['value'] == update_street_name(child.attrib['v'])
                            
            elif node_tag['key'] == 'phone':
                node_tag['value'] = update_phone_number(child.attrib['v'])
        ...
        for child in element:       
            ...
            if child.tag == 'nd':
                ...
                if LOWER_COLON.match(ktag):
                    c = ktag.find(':')
                    way_tag['key'] = ktag[c+1:]
                    way_tag['type'] = ktag[:c+1]
